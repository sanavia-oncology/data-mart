...
merge_order_sheets = function(order_sheets) {
    ...
    oid_cs = clone_strategy[["Order ID"]]
    oid_os = order_summary[["Order ID"]]
    
    if (any(duplicated(oid_cs))) {
        msg = "clone_strategy 'Order ID' contains duplicates (K.Okrah)\n"
        cat(msg)
        stop()
    }
    
    if (any(duplicated(oid_os))) {
        msg = "order_summary 'Order ID' contains duplicates (K.Okrah)\n"
        cat(msg)
        stop()
    }
    
    if (!length(oid_cs)==length(oid_os)) {
        msg = paste0("clone_strategy 'Order ID' and ",
                     "order_summary 'Order ID' lengths do not match!",
                     " (K.Okrah)\n")
        cat(msg)
        stop()
    }
    if (!all(sort(oid_cs)==sort(oid_os))) {
        msg = paste0("clone_strategy 'Order ID' and ",
                     "order_summary 'Order ID' names do not match!",
                     " (K.Okrah)\n")
        cat(msg)
        stop()
    }
    ...
    # 2. add location map
    oid_loc = location_map[["Order ID"]]
    
    if (any(duplicated(oid_loc))) {
        msg = "location_map 'Order ID' contains duplicates (K.Okrah)\n"
        cat(msg)
        stop()
    }
    
    if (length(oid_cs) < length(oid_loc)) {
        msg = paste0("location_map 'Order ID' is longer than ",
