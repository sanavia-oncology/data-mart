...
preproc_location_map_00 = function(df) {  
    ...
    for (k in names(order_id_index)) {
        ...
        for (i in 1:n) {
            ...
            bt_check = grep("Wells Box|DeNiro", bt)
...
flatten_location_map = function(location_map_df) {
    
    location_map_df_list_ = split(location_map_df, location_map_df[["Name"]])
    location_map_df_list = location_map_df_list_
    
    for (k in names(location_map_df_list)) {
        x = location_map_df_list[[k]]
        if (nrow(x) > 1) {
            y = x[1,,drop=F]
            for (j in c("Order ID", "Name", "Box Name", 
                        "Position", "Volume(ml)", "Box Type")) {
                if (j %in% c("Order ID", "Name")) {
                    z = unique(x[,j])
                }else{
                    z = paste0(x[,j], collapse = " | ")
                }
                y[,j] = z
            }
            location_map_df_list[[k]] = y
        }
    }
    
    location_map_df1 = do.call(rbind, location_map_df_list)

    rownames(location_map_df1) = NULL
    
    return(location_map_df1)
}
